				...
				if($num_rows_l3)
				{
					for($i_l3=1;$i_l3<=$num_rows_l3;$i_l3++)
					{
						...
						else
						{
							...
							foreach ($vids as $vid)
							{
								$query_l4 = "select * from bhashya where bid='$bhashya' and level='4' and id='$vid'";
								$result_l4 = mysql_query($query_l4);
								$num_rows_l4 = mysql_num_rows($result_l4);
								$row_l4=mysql_fetch_assoc($result_l4);

								$id=$row_l4['id'];
								$type=$row_l4['type'];
								$title=$row_l4['title'];
								$content=$row_l4['content'];
								
								if(($type == "shloka") || ($type == "sutra") || ($type == "mantra"))
								{
									echo "<div class=\"verse hashnav\" id=\"$id\" type=\"$type\">";
									//~ $content=InsertIdRef($content);
									echo ProcessRef($content, $hval) . "\n";
									
									$query_l4b = "select * from bhashya where bid='$bhashya' and level='4b' and id regexp '^$id'";
									$result_l4b = mysql_query($query_l4b);
									$num_rows_l4b = mysql_num_rows($result_l4b);
									...
									if($num_rows_l4b)
									{
										...
										for($i_l4b=1;$i_l4b<=$num_rows_l4b;$i_l4b++)
										{
											$row_l4b=mysql_fetch_assoc($result_l4b);

											$content=$row_l4b['content'];
											//~ $content=InsertIdRef($content);
											echo ProcessRef($content, $hval) . "\n";
										}
										echo "</div>";
